...
import { supabase, type JobTitle } from "@/lib/supabase";
import EntityRow from "./EntityRow";

type Props = {
  companyId: string;
  data: JobTitle[];
  onChange: (next: JobTitle[]) => void;
};

export default function JobTitlesPanel({ companyId, data, onChange }: Props) {
  ...
  const handleDelete = async (row: JobTitle) => {
    if (!confirm(`"${row.name}" pozisyonunu silmek istediğine emin misin?`))
      return;
    ...
    <div className="space-y-4">
      ...
      >
        <div className="flex-1 space-y-1.5">
          <Label htmlFor="new-jobtitle">Yeni Pozisyon</Label>
          <Input
            id="new-jobtitle"
            value={name}
    ...
    >
      <form onSubmit={handleSave} className="space-y-4">
        <div className="space-y-1.5">
          <Label htmlFor="edit-jobtitle-name">Ad</Label>
          <Input
            id="edit-jobtitle-name"
            value={name}
            onChange={(e) => setName(e.target.value)}
            disabled={saving}
            autoFocus
